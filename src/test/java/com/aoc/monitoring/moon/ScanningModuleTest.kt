...
class ScanningModuleTest {
    ...
    fun scanMoonsExampleData() {
        val data = InputReader().readInputAsString("/moon/example-moon-positions.txt").values
        val moonPositions = ScanningModule().scanLocalSectorForMoons(data)
        assertThat(moonPositions).isEqualTo(setOf(
                Moon(Point3D(-1,0,2), Velocity3D(0,0,0)),
                Moon(Point3D(2,-10,-7), Velocity3D(0,0,0)),
                Moon(Point3D(4,-8,8), Velocity3D(0,0,0)),
                Moon(Point3D(3,5,-1), Velocity3D(0,0,0))
        ))
    }
}