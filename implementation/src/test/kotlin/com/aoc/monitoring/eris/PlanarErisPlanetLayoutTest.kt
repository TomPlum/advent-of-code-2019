...
class PlanarErisPlanetLayoutTest {
    ...
    fun getDyingBugsExampleOne() {
        ...
        val scan = PlanarErisPlanetLayout(scanData.value)
    ...
    fun equalityTestNegative() {
        val firstScanData = TestInputReader().readInputAsString("/eris/example-scan-1.txt").value
        val firstScan = PlanarErisPlanetLayout(firstScanData)

        val secondScanData = TestInputReader().readInputAsString("/eris/biodiversity-example.txt").value
