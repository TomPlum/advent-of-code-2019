...
class SmallShuffleInstructionParserTest {
    @Test
    fun dealWithIncrement() {
        val instructions = listOf("deal with increment 7")
        val strategies = SmallShuffleInstructionParser().parse(instructions)
    ...
    fun invalidInstruction(instruction: String) {
        val e = assertThrows<IllegalArgumentException> { SmallShuffleInstructionParser().parse(listOf(instruction)) }
